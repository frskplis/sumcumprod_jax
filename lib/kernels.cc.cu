...
template <typename T>
__global__ void sumcumprod_kernel_masked(std::int64_t size, int size_of_last_dim, const T *input_array, const T *rs, T *output_array) {
  ...
  float multi_cur = 0.0;
      ...
      multi_cur = multi_prev * multi_cur;
      multi_prev = multi_cur;
      total_sum += multi_cur * (input_array[i] != 0.f);
	  }
	  output_array[tid_start] = total_sum;
...
template <typename T>
inline void apply_sumcumprod_masked(cudaStream_t stream, void **buffers, const char *opaque,
                         std::size_t opaque_len) {
  const sumcumprodDescriptor &d = *UnpackDescriptor<sumcumprodDescriptor>(opaque, opaque_len);
  const std::int64_t size = d.size1;
  const std::int64_t size_of_last_dim = d.size2;
...
void gpu_sumcumprod_masked_f32(cudaStream_t stream, void **buffers, const char *opaque,
                    std::size_t opaque_len) {
  apply_sumcumprod_masked<float>(stream, buffers, opaque, opaque_len);
}
...
void gpu_sumcumprod_masked_f64(cudaStream_t stream, void **buffers, const char *opaque,
                    std::size_t opaque_len) {
  apply_sumcumprod_masked<double>(stream, buffers, opaque, opaque_len);
}
